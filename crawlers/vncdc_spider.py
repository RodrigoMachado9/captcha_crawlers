...
class CaptchaSpider(InitSpider):
    name = "captcha_spider"
    start_urls = ['https://tiemchung.vncdc.gov.vn/DichVuVas/NapTien/GetCaptcha']
    rules = (
    ...
    def post_init_requests(self):
        ...
        while count < self.max_count:
            yield scrapy.Request('https://tiemchung.vncdc.gov.vn/DichVuVas/NapTien/GetCaptcha', callback=self.parse, dont_filter=True)
            count += 1
            time.sleep(self.sleep_time)

    def parse(self, response):
        file_name = os.path.join(self.crawled_dir, 'img_' + str(time.time()) + '.gif')
    ...
    def check_login_response(self, response):
        ...
        if self.http_user in response.body:
            ...
            print response.css('#resetValText').extract()[0]
            self.log("Bad times :(")
...
if __name__ == "__main__":
    ...
    params = dict(url='https://tiemchung.vncdc.gov.vn/DichVuVas/NapTien/GetCaptcha',
                  login_page='https://tiemchung.vncdc.gov.vn/Account/Login',
                  http_user='ls_hl_thitranhuulung',
                  http_password='123456a@Xa',
                  crawled_dir='/home/thieunguyen/Datasets/captcha/vncdc/raw_gif_003',
                  max_count=700)
    print params
